...
passes the family's unit-direction door: it must be finite and within `1e-6` of
unit norm, and is then projected exactly onto the sphere — float noise is
repaired, but a column that is not a direction (a moment-scaled vector,
`‖e‖ = 1.7`) is refused rather than silently normalized. Normalize deliberately
in your own code if that is what you mean.
"""
